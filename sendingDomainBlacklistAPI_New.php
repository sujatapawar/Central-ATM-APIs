...
if(isset($jsonString) and $jsonString!="")
{
	...
	$poolId = $Req1_Details[0]['assigned_priority']; $poolName = $Env_ID[0]['pool_name'];$agencyId=$jsonData['agency_id'];
	$obj->putAssetLog($mainDomainId[0]['domain_id'],2,"Domain Blacklisted","Req1=$obj->req1,Domain Name=$main_domain,Domain Type=$domainType,Pool Id=$poolId,Pool Name=$poolName,agency id=$agencyId,Captured By=ATM2");
	...
	if(!isset($freezerIPArray[0]['IP_id'])) // check if IP is in freezer
	{
		...
		$obj->putAssetLog($ipIds[0]['IP_id'],1,"IP removed from pool","Req1=$obj->req1,IP=".$IPArray[0]['IP'].",Reason=Domain $main_domain($domainType) got blacklisted,Pool Id=$poolId,Pool Name=$poolName,agency id=$agencyId,Done by=ATM2");

		//put Ip in available_assets pool
		$obj->putAssetIntoAvailablePool($ipIds[0]['IP_id']);	
		$obj->putAssetLog($ipIds[0]['IP_id'],1,"IP put into Available Assets","Req1=$obj->req1,IP=".$IPArray[0]['IP'].",Reason=Domain $main_domain($domainType) got blacklisted,Pool Id=$poolId,Pool Name=$poolName,agency id=$agencyId,Done by=ATM2");
	}	
