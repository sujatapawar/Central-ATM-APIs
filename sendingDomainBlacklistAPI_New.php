...
if(isset($jsonString) and $jsonString!="")
{
	...
	$obj->putAssetLog($mainDomainId[0]['domain_id'],"Domain Blacklisted","Domain blacklist by agency id : $jsonData['agency_id']");
	...
	$obj->_dbHandlepdo->sql_delete("Domain_MTA_mapping", " where domain_id=?", array($domain['domain_id']));
        $obj->putAssetLog($domain['domain_id'],"Domain removed from domain_master","Domain removed from domain_master by ATM2 due to domain blacklist");
	...
	$obj->putAssetLog($mainDomainId[0]['domain_id'],"Domain put into freezer","Domain put into freezer by ATM2 due to domain blacklist");
	$obj->logBlacklistingTransactions($mainDomainId[0]['domain_id'],$jsonData['agency_id']);
	//Releasing IP
	$IPID = $obj->releaseIP();
	$IPRelease = array();
	$obj->connection_atm();
